  ...
  <div class="main-layout">
    ...
    <section class="stay-list">   
      <div class="trip-card-holder" v-for="trip in myTrips" :key="trip._id"  >
            <div class="trip-card-img-container"><img :src="stayPreviewImage(trip)" class="trip-card-img" /></div>
            <div class="trip-text-container">
              <div class="trip-host-details bold">{{ trip.hostName }}
              <div style="opacity: 0.6">{{ trip.hostType }} hosted by {{trip.hostedBy }}</div>
              </div>
              ...
              <div class="trip-footer clickable" @click="openDetails(trip.stayId)">
                <img :src ="tripIcon()" style="width:38px; height: 34px; margin-right: 10px;" />
                <div><span class="bold">Your host details:</span><span class="pl-5">Gettng there, getting inside</span></div>
              </div>
          </div>
      </div>
